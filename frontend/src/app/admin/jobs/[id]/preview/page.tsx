...
import {
  ...
  TrendingUp, BadgeCheck, Users, GraduationCap,
  CheckCircle2, Building2, Clock, Calendar,
  Wifi, Monitor, Tag, Star,
} from "lucide-react";
...
export default async function AdminJobPreviewPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <DashboardShell role="ADMIN" current="/admin/jobs">
      ...
      <div className="mb-6">
        <div>
          <Link href="/admin/jobs" className="inline-flex items-center gap-1.5 text-sm text-zinc-500 hover:text-zinc-800 transition mb-1">
            <ArrowLeft className="h-4 w-4" /> Back to Jobs
          </Link>
          <h1 className="text-2xl font-black text-zinc-900">Job Preview</h1>
          <p className="text-sm text-zinc-400 mt-0.5">Posted by {job.postedBy.name} · {job.company.name}</p>
        </div>
      </div>

      {/* Main grid */}
      <div className="grid gap-5 pb-10 lg:grid-cols-[1fr_280px]">
        ...
        <div className="space-y-4">
          ...
          <div className="bg-white border border-zinc-100 rounded-2xl p-6 shadow-sm">
            <div className="flex items-start gap-3 mb-4">
              ...
              <span className="px-3 py-1 rounded-full bg-zinc-50 text-zinc-700 text-xs font-semibold border border-zinc-200">
                {job.status}
              </span>
            </div>
            ...
            <div className="mt-3 flex flex-wrap gap-4 text-xs text-zinc-500">
              ...
              <span className="flex items-center gap-1"><Clock className="h-3.5 w-3.5" /> Posted on {job.publishedAt ? new Date(job.publishedAt).toLocaleDateString("en-IN", { day: "2-digit", month: "short", year: "numeric" }) : new Date(job.createdAt).toLocaleDateString("en-IN", { day: "2-digit", month: "short", year: "numeric" })}</span>
          ...
          {job.jobSkills.length > 0 && (
            <div className="bg-white border border-zinc-100 rounded-2xl p-6 shadow-sm">
              <h3 className="text-base font-bold text-zinc-900 mb-1">{job.keySkills.length > 0 ? "Key Skills" : "Skills"}</h3>
              {job.keySkills.length > 0 && (
                <p className="text-xs text-zinc-400 mb-3">Skills highlighted with ★ are preferred key skills.</p>
              )}
              <div className="flex flex-wrap gap-2">
                {[...job.jobSkills].sort((a, b) => (job.keySkills.includes(a.skill.name) ? 0 : 1) - (job.keySkills.includes(b.skill.name) ? 0 : 1)).map(s => {
                  const isKey = job.keySkills.includes(s.skill.name);
                  return (
                    <span key={s.skill.id} className={`text-xs px-3 py-1.5 rounded-full font-medium border ${isKey ? "bg-amber-50 text-amber-700 border-amber-200" : "bg-blue-50 text-blue-700 border-blue-100"}`}>
                      {isKey && "★ "}{s.skill.name}
                    </span>
                  );
                })}
          ...
          <div className="bg-white border border-zinc-100 rounded-2xl p-5 shadow-sm">
            ...
            {job.status === "PUBLISHED" && (
              <Link href={`/jobs/${job.slug}`} target="_blank"
                ...
                <CheckCircle2 className="h-4 w-4" /> View Public Listing
              </Link>
            )}
          </div>
        </div>
